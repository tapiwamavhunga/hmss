...
class InvoiceTable extends LivewireTableComponent
{
    ...
    public function columns(): array
    {
        ...
        return [
            ...
            Column::make(__('messages.invoice.patient'), 'patient.patientUser.first_name')
                ...
                ->sortable()
                ->searchable(),
            Column::make(__('messages.invoice.patient'), 'patient.patientUser.last_name')->searchable()->hideIf(1),
    ...
    public function builder(): Builder
    {
        if (! getLoggedinPatient()) {
            ...
            $query = Invoice::where('invoices.tenant_id', '=',getLoggedInUser()->tenant_id)->whereHas('patient.patientUser')->with(['patient.patientUser'])->select('invoices.*')->where('patient_id',
                $patientId->id);
        }
        ...
        $query->when(isset($this->statusFilter), function (Builder $q) {
            ...
            if ($this->statusFilter == 0) {
                $q->where('invoices.status', Invoice::PENDING);
            }
        });

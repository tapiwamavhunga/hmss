...
class VaccinatedPatientTable extends LivewireTableComponent
{
    ...
    public function columns(): array
    {
        if (! Auth::user()->hasRole(['Patient'])) {
            ...
            $actionButton = Column::make(__('messages.patient_diagnosis_test.action'), 'id')
                ->view('vaccinated_patients.action');
        } else {
            $this->showButtonOnHeader = false;
            $actionButton = Column::make(__('messages.patient_diagnosis_test.action'), 'id')
                ->view('patient_diagnosis_test.vaccinated_patients.action')
                ->hideIf(1);
        }

        return [
            Column::make(__('messages.vaccinated_patient.patient'), 'patient.patientUser.first_name')
                ->view('vaccinated_patients.column.patient')
                ->sortable()
            ...
            Column::make(__('messages.vaccinated_patient.vaccination'), 'vaccination.name')
                ->view('vaccinated_patients.column.vaccination')
                ->sortable()
                ->searchable(),
            Column::make(__('messages.vaccinated_patient.serial_no'), 'vaccination_serial_number')
                ->view('vaccinated_patients.column.serial_no')
                ->sortable()
                ->searchable(),
            ...
            Column::make(__('messages.vaccinated_patient.does_no'), 'dose_number')
                ...
                ->sortable()
                ->searchable(),
            Column::make(__('messages.vaccinated_patient.dose_given_date'), 'dose_given_date')
                ->view('vaccinated_patients.column.dose_given_date')
                ->sortable(),
            $actionButton,
        ];
    }

...
class PatientDiagnosisTest extends Model
{
    ...
    public function convertToPdf($id)
    {
        ...
        $data = $patientDiagnosisTestRepository->getSettingListForPDF();
        ...
        if (Storage::disk(config('app.media_disc'))->exists('diagnosis/Diagnosis-'.$this->report_number.'.pdf')) {
            Storage::disk(config('app.media_disc'))->delete('diagnosis/Diagnosis-'.$this->report_number.'.pdf');
        }

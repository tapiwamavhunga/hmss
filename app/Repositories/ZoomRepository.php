...
class ZoomRepository
{
    ...
    public function connectWithZoom($code)
    {
        ...
        if (isset($userZoomCredential)) {
            ...
            return Flash::error(__('messages.new_change.add_credential'));
        }
    ...
    public function updateZoomMeeting($data,$liveConsultation)
    {
        ...
        $data = json_decode($response->getBody());
            ...
            if($e->getCode() == 401){
                throw new UnprocessableEntityHttpException(__('messages.new_change.invalid_token'));
            }
            if($e->getCode() == 0){
                throw new UnprocessableEntityHttpException(__('messages.new_change.have_to_connect_zoom'));
            }
    ...
    public function createZoomMeeting($data)
    {
        ...
        try {
            ...
            if (401 == $e->getCode()) {
                if(!isset($clientID)){
                    throw new UnprocessableEntityHttpException(__('messages.new_change.already_in_use'));
                }
                $userZoomCredential = UserZoomCredential::where('user_id', getLoggedInUserId())->first();
                if (!isset($userZoomCredential)) {

                    throw new UnprocessableEntityHttpException(__('messages.new_change.add_credential'));
                }
    ...
    public function destroyZoomMeeting($meetingId){
        ...
        try {
            ...
            if($e->getCode() == 400){
                throw new UnprocessableEntityHttpException(__('messages.new_change.in_progress'));
            }
            if($e->getCode() == 0){
                throw new UnprocessableEntityHttpException(__('messages.new_change.have_to_connect_zoom'));
            }

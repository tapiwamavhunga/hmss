...
function loadIpdDiagnosisData () {
    if (!$('#ipdDiagnosisReportDate').length &&
...
listen('click', '.ipdDignosis-delete-btn', function (event) {
    ...
    deleteItem($('#showIpdDiagnosisUrl').val() + '/' + id, '',
        $('#ipdDiagnosisLang').val())
})
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                document.querySelector('#editIpdDiagnosisReportDate').
                    _flatpickr.
                    setDate(moment(result.data.report_date).format())
                $('#editIpdDiagnosisDescription').val(result.data.description)
                if (result.data.ipd_diagnosis_document_url != '') {
                    $('#editIpdDiagnosisDocumentViewUrl').show()
                    $('.btn-view').show()
                    $('#editIpdDiagnosisDocumentViewUrl').
                        attr('href', result.data.ipd_diagnosis_document_url)
                } else {
...
listenChange('#ipdDiagnosisDocumentImage', function () {
    let extension = isValidIpdDiagnosisDocument($(this),
...
function isValidIpdDiagnosisDocument (

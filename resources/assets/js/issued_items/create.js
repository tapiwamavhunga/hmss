...
function loadIssuedItems() {
    ...
    $("#issueItems").select2({
        placeholder: Lang.get("js.select_item"),
        width: "100%",
    });
...
listenChange("#issueItemCategory", function () {
    ...
    $("#issueItems").empty();
    // $('#issueItems').append('<option>'+Lang.get('messages.new_theme.select_item')+'</option>')
    $("#issueItems").prop("disabled", true);
});
...
listenChange("#itemQuantity", function () {
    ...
    if (quantity <= availableQuantity) {
        ...
        showIssueItemError(Lang.get("js.qty_cannot_be_zero"));
    else showIssueItemError(Lang.get("js.qty_must_be_less_than_available_qty"));
});

            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="form-group col-sm-6 mb-5">
                        ...
                        {{ Form::text('title', null, ['class' => 'form-control','required', 'placeholder' => __('messages.ipd_patient_timeline.title')]) }}
                    </div>
                    <div class="form-group col-md-6 mb-0">
                        <div class="form-group mb-5">
                            {{ Form::label('date', __('messages.ipd_patient_timeline.date').':',['class' => 'form-label']) }}
                            <span class="required"></span>
                            {{ Form::text('date', null, ['class' => (getLoggedInUser()->thememode ? 'bg-light form-control' : 'bg-white form-control'),'id' => 'ipdTimelineDate','autocomplete' => 'off', 'required', 'placeholder' => __('messages.ipd_patient_timeline.date')]) }}
                        </div>
                    </div>
                    <div class="form-group col-md-6 mb-0">
                        <div class="form-group mb-5">
                            {{ Form::label('description', __('messages.ipd_patient_timeline.description').':',['class' => 'form-label']) }}
                            {{ Form::textarea('description', null, ['class' => 'form-control', 'rows' => 4, 'placeholder' => __('messages.ipd_patient_timeline.description')]) }}
                        </div>
                    </div>
                        ...
                        <div class="d-block">
                            <div class="image-picker">
                                ...
                                <span class="picker-edit rounded-circle text-gray-500 fs-small"
                                      title="{{ __('messages.ipd_patient_timeline.document') }}">
                            <label>
                                <i class="fa-solid fa-pen" id="profileImageIcon"></i>
                                <input type="file" id="ipdTimelineDocumentImage" name="attachment"
                                       class="image-upload d-none" accept=".png, .jpg, .jpeg, .gif, .pdf, .webp"/>
                            </label>
                            </span>
                            </div>
                        </div>

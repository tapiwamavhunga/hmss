<div class="container-fluid">
            ...
            <div class="row">
                <div class="col-sm-6 col-12">
                    <div class="mb-sm-0 mb-6">
                        ...
                        {{ Form::select('patient_id', $patients, isset($invoice) ? $invoice->patient_id : null, ['class' => 'form-select', 'id' => 'patient_id', 'placeholder' => __('messages.document.select_patient'),'required','data-control' =>'select2']) }}
                    </div>
                </div>
                <div class="col-sm-6 col-12">
                    @if(isset($invoice))
                        <div class="">
                            ...
                            <span class="required"></span>
                            {{ Form::text('invoice_date', null, ['class' => (getLoggedInUser()->thememode ? 'bg-light form-control' : 'bg-white form-control'), 'id' => 'editInvoiceDate', 'autocomplete' => 'off','required', 'placeholder' =>  __('messages.invoice.invoice_date')]) }}
                        </div>
                    @else
                        <div class="">
                            {{ Form::label('invoice_date', __('messages.invoice.invoice_date').(':'),['class' => 'form-label']) }}
                            <span class="required"></span>
                            {{ Form::text('invoice_date', null, ['class' => (getLoggedInUser()->thememode ? 'bg-light form-control' : 'bg-white form-control'), 'id' => 'invoice_date', 'autocomplete' => 'off','required', 'placeholder' =>  __('messages.invoice.invoice_date')]) }}
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div class="mb-0">
        <div class="row gx-10 mb-5">
            <div class="col-lg-6 col-sm-12">
                <div class="mb-5">
                    ...
                    (%)
                    <span class="required"></span>
                    {{ Form::number('discount',  isset($invoice) ? $invoice->discount : null, ['id'=>'discount','class' => 'form-control','placeholder' => __('messages.document.in_percentage'),'required', 'min' => 0, 'max' => 100, 'step' => '.01']) }}
                </div>
            </div>
            <div class="col-lg-6 col-sm-12">
                <div class="mb-5">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 d-flex justify-content-end">
            <button type="button" class="btn btn-primary text-star addInvoiceItem"
                    id="addInvoiceItem"> {{ __('messages.invoice.add') }}</button>
        </div>
            ...
            <table class="table g-5 gs-0 mt-2" id="billTbl">
                ...
                @if(isset($invoice))
                    ...
                    @foreach($invoice->invoiceItems as $invoiceItem)
                        <tr class="border-bottom">
                            ...
                            <td class="text-center">
                                {{ Form::text('price[]', number_format($invoiceItem->price,2), ['class' => 'form-control decimal-number price','required', 'placeholder' => __('messages.invoice.price')]) }}
                            </td>
                            <td class="amount text-center item-total pt-5 ms-2 text-nowrap">
                                {{--                                <span>{{ getCurrencySymbol() }}</span>--}}
                                <span class="amount item-total">{{ number_format($invoiceItem->total,2) }}</span>
                            </td>
                            <td class="text-end">
                ...
                @else
                    <tr class="border-bottom border-bottom-dashed">
                        ...
                        <td class="table__qty">
                            {{ Form::number('quantity[]', null, ['class' => 'form-control qty','required', 'type' => 'number', "min" => 1, 'placeholder' => __('messages.invoice.qty')]) }}
                        </td>
                        <td>
                            {{ Form::text('price[]', null, ['class' => 'form-control decimal-number price','required', 'placeholder' => __('messages.invoice.price')]) }}
                        </td>

        ...
        <div class="row">
            ...
            <div class="col-md-3 col-6 text-end">
                <a target="_blank" href="{{ route('invoices.pdf',['invoice' => $invoice->id]) }}"
                   class="btn btn-success text-white mt-5">
                    {{ __('messages.invoice.print_invoice') }}
                </a>
                <button class="btn btn-success text-white mt-5 sendInvoiceBtn">
                    {{ __('messages.invoice.send_mail') }}
                </button>
            </div>
            <div class="col-lg-5 col-md-6">
                ...
                <table class="table table-striped box-shadow-none mt-4">
                    ...
                    @foreach($invoice->invoiceItems as $index => $invoiceItem)
                        <tr class="text-end">
                            ...
                            <td class="pt-6">{{ getCurrencyFormat($invoiceItem->price) }}
                            </td>
                ...
                <div class="border-top">
                    <table class="table table-borderless {{ getLoggedInUser()->thememode ? '' : 'bg-white' }} box-shadow-none mb-0 mt-5">
                        ...
                        <tr>
                            ...
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 text-end pe-0">{{ getCurrencyFormat($invoice->amount) }}</td>
                        </tr>
                        <tr>
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} ps-0">{{ __('messages.invoice.discount').(':') }}</td>
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 text-end pe-0">{{ getCurrencyFormat(($invoice->amount * $invoice->discount / 100)) }}</td>
                        </tr>
                        <tr>
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} ps-0">{{ __('messages.invoice.total').(':') }}</td>
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 text-end pe-0">{{ getCurrencyFormat($invoice->amount - ($invoice->amount * $invoice->discount / 100))}}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="bg-light-100 rounded-15 p-md-7 p-5 h-100 mt-xxl-0 mt-5 col-xxl-9 ms-xxl-auto">
            <div class="mb-8">
                @if($invoice->status == \App\Models\Invoice::PENDING)
                    <span class="badge bg-light-warning">{{ __('messages.new_change.pending_payment') }}</span>
                @elseif($invoice->status == \App\Models\Invoice::PAID)
                    <span class="badge bg-light-success me-2">{{ __('messages.invoice.paid') }}</span>
                @endif
            </div>
            <h6 class="mb-5">{{  ucfirst(trans('messages.advanced_payment.patient').' '.trans('messages.overview')) }}</h6>
            <div class="mb-6">
                ...
                <div class="fs-5 text-gray-900">
                    <a href="{{ route('patients.show', ['patient' => $invoice->patient->id]) }}"
                       class="link-primary fs-5 text-decoration-none">{{ $invoice->patient->patientUser->full_name }}</a>
                </div>
            </div>
            <div class="mb-6">

<div class="row gx-10 mb-5">
    ...
    @else
        <div class="form-group col-sm-4 mb-5">
            {{ Form::label('per_patient_time', __('messages.schedule.per_patient_time') . ':', ['class' => 'form-label']) }}
            <span class="required"></span>
            {{ Form::text('per_patient_time', null, ['id' => 'perPatientTime', 'class' => 'form-control perPatientTime' . (getLoggedInUser()->thememode ? ' bg-light' : ' bg-white'), 'required', 'placeholder' => __('messages.schedule.per_patient_time')]) }}
        </div>
    @endif

    <div class="table table-responsive col-lg-12 col-md-12 col-sm-12 schedulesCon">
                    ...
                    <tr>
                        <td class="schedules-table-td">
                            {{ Form::text(
                                'available_on_days[]',
                                isset($scheduleDays)
                                    ? __('messages.schedule_weekday.' . $scheduleDays[$loop->iteration - 1]->available_on)
                                    : __('messages.schedule_weekday.' . $days),
                                ['class' => 'form-control availableOn', 'required', 'id' => 'availableOn-' . ($loop->iteration - 1), 'readonly'],
                            ) }}
                            {{ Form::hidden('available_on[]', isset($scheduleDays) ? $scheduleDays[$loop->iteration - 1]->available_on : $days) }}
                        </td>
                        ...
                        <td class="schedules-table-td position-relative">
                            {{ Form::text(
                                'available_to[]',
                                isset($scheduleDays) ? $scheduleDays[$loop->iteration - 1]->available_to : '00:00:00',
                                [
                                    'id' => 'availableTo-' . ($loop->iteration - 1),
                                    'class' =>
                                        'form-control availableTo hospitalScheduleTo-' .
                                        $loop->iteration .
                                        (getLoggedInUser()->thememode ? ' bg-light' : ' bg-white'),
                                    'required',
                                    'autocomplete' => 'off',
                                ],
                            ) }}
                        ...
                        <td class="text-center schedules-table-td">
                            ...
                            <a title="copy-previous"
                                class="btn action-btn btn-primary btn-sm copy-btn cpy-btn{{ $loop->iteration - 1 }}"
                                href="javascript:void(0)" data-id="{{ $loop->iteration - 1 }}">
...
<div class="d-flex justify-content-end">
    {!! Form::submit(__('messages.common.save'), [
        ...
        'id' => 'scheduleSave',
        $data['hospitalSchedule'] == null ? 'disabled' : '',
    ]) !!}

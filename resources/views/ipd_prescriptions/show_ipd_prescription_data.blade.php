...
<div class="row">
    <div class="form-group col-sm-12 my-0">
        <a href="{{ route('ipd.prescriptions.pdf',$ipdPrescription->id) }}" class="btn btn-sm btn-success float-end" target="_blank">
...
<div id='DivIdToPrint'>
    ...
    <div class="row g-5 mb-11">
        ...
        <div class="col-sm-2">
            ...
            <div class="fw-bolder fs-6 text-gray-800 text-wrap text-break">{{ $ipdPrescription->patient->patient->user->email }}</div>
        </div>
        <div class="col-sm-2">
            <div class="fw-bold pe-10 text-gray-600 fs-7">{{ __('messages.user.phone').':' }}</div>
            <div
                    class="fw-bolder fs-6 text-gray-800">{{ $ipdPrescription->patient->patient->user->phone != null ? $ipdPrescription->patient->patient->user->region_code.$ipdPrescription->patient->patient->user->phone : __('messages.common.n/a') }}</div>
        </div>
        <div class="col-sm-2">
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer w-100 mb-3">
            ...
            @foreach($ipdPrescription->ipdPrescriptionItems as $ipdPrescriptionItem)
                <tr class="fw-bolder text-gray-700 fs-5 text-end">
                    <td class="text-start pt-6">{{ $ipdPrescriptionItem->medicineCategory->name }}
                        - {{$loop->iteration}}</td>
                    ...
                    <td class="text-start pt-6">{{ App\Models\Prescription::DOSE_INTERVAL[$ipdPrescriptionItem->dose_interval] ?? 'N\A' }}</td>

            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="form-group col-sm-6 mb-5">
                        ...
                        {{ Form::text('from_title', null, ['class' => 'form-control editFromTitle','required','id' => 'editReceiveFromTitle', 'placeholder' => __('messages.postal.from_title')]) }}
                    </div>
                    <div class="form-group col-sm-6 mb-5">
                        {{ Form::label('Reference Number',__('messages.postal.reference_no').':', ['class' => 'form-label']) }}
                        {{ Form::text('reference_no', null, ['class' => 'form-control editReferenceNumber','id' => 'editReceiveReferenceNumber', 'placeholder' => __('messages.postal.reference_no')]) }}
                    </div>
                    <div class="form-group col-sm-6 mb-5">
                        {{ Form::label('date', __('messages.postal.date').':', ['class' => 'form-label']) }}
                        {{ Form::text('date', null, ['class' => (getLoggedInUser()->thememode ? 'bg-light editPostalDate form-control' : 'bg-white editPostalDate form-control'),'autocomplete' => 'off','id' => 'editReceiveDate', 'placeholder' => __('messages.postal.date')]) }}
                    </div>
                    <div class="form-group col-sm-6 mb-5">
                        {{ Form::label('To Title',__('messages.postal.to_title').':', ['class' => 'form-label']) }}
                        {{ Form::text('to_title', null, ['class' => 'form-control editToTitle','id' => 'editReceiveToTitle', 'placeholder' => __('messages.postal.to_title')]) }}
                    </div>
                                    ...
                                    <span class="picker-edit rounded-circle text-gray-500 fs-small"
                                          title="Change Attachment">
                                            <label>
                                                ...
                                                <input type="file" id="editReceiveAttachments" name="attachment"
                                                       class="image-upload d-none editAttachments"
                ...
                <div class="form-group col-sm-12 mb-5">
                    ...
                    {{ Form::textarea('address', null, ['class' => 'form-control editAddress', 'rows' => 4, 'id' => 'editReceiveAddress', 'placeholder' => __('messages.postal.address')]) }}
                </div>
            </div>
        </div>

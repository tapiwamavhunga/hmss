            ...
            <tr>
                <td style="width: 1%;" class="py-1">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="chkShortWeekDay_{{$shortWeekDay}}"
                               value="{{$day}}" name="checked_week_days[]" @if($isValid) checked="checked" @endif>
                    </div>
                </td>
                <td style="width: 1%;" class="py-1">
                    <label class="form-check-label" for="chkShortWeekDay_{{$shortWeekDay}}">
                        <span class="fs-5 fw-bold d-md-block d-none">{{__('messages.hospital_schedule_weekday.'.$shortWeekDay)}}</span>
                    </label>
                </td>
...
<div class="d-flex justify-content-end">
    {{ Form::button(__('messages.common.save'), ['type' => 'submit','class' => 'btn btn-primary', 'id' => 'btnHospitalScheduleSave','data-loading-text' => "<span class='spinner-border spinner-border-sm'></span> Processing..."]) }}
</div>

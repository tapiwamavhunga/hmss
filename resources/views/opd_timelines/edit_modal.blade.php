            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="form-group col-sm-6 mb-5">
                        ...
                        {{ Form::text('title', null, ['class' => 'form-control','required' , 'id' => 'editOPdTimelineTitle', 'placeholder' => __('messages.ipd_patient_timeline.title')]) }}
                    </div>
                    <div class="form-group col-md-6 mb-5">
                        <div class="form-group">
                            {{ Form::label('date', __('messages.ipd_patient_timeline.date').':',['class' => 'form-label']) }}
                            <span class="required"></span>
                            {{ Form::text('date', null, ['class' => (getLoggedInUser()->thememode ? 'bg-light form-control' : 'bg-white form-control'),'id' => 'editOPdTimelineDate','autocomplete' => 'off', 'required', 'placeholder' => __('messages.ipd_patient_timeline.date')]) }}
                        </div>
                    </div>
                    <div class="form-group col-md-6 mb-5">
                        <div class="form-group">
                            {{ Form::label('description', __('messages.ipd_patient_timeline.description').':',['class' => 'form-label']) }}
                            {{ Form::textarea('description', null, ['class' => 'form-control', 'rows' => 4, 'id' => 'editOpdTimelineDescription', 'placeholder' => __('messages.ipd_patient_timeline.description')]) }}
                        </div>
                    </div>

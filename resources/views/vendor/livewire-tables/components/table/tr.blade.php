...
<tr rowpk='{{ $row->{$component->getPrimaryKey()} }}' x-on:dragstart.self="currentlyReorderingStatus && dragStart(event)"
    ...
    @if ($component->hasDisplayLoadingPlaceholder()) wire:loading.remove
    @else
    wire:loading.class.delay="opacity-50 dark:bg-gray-900 dark:opacity-60" @endif
    id="{{ $tableName }}-row-{{ $row->{$component->getPrimaryKey()} }}" :draggable="currentlyReorderingStatus"

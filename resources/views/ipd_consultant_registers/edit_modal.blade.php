            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="form-group col-md-12 mb-5">
                        ...
                        {{ Form::text('applied_date', null, ['class' => 'form-control appliedDate bg-white min-w-170 modelDataPickerzindex', 'id' => 'editConsultantAppliedDate', 'autocomplete' => 'off', 'required', 'placeholder' => __('messages.ipd_patient_consultant_register.applied_date')]) }}
                    </div>
                    <div class="form-group col-sm-12 mb-5">
                        {{ Form::label('doctor_id', __('messages.ipd_patient_consultant_register.doctor_id').':',['class' => 'form-label']) }}
                        <span class="required"></span>
                        {{ Form::select('doctor_id', $doctors, null, ['class' => 'form-select doctorId select2Selector','required','placeholder'=> __('messages.web_home.select_doctor'), 'id' => 'editConsultantDoctorId']) }}
                    </div>
                    <div class="form-group col-md-12 mb-5">
                        {{ Form::label('instruction_date', __('messages.ipd_patient_consultant_register.instruction_date').':',['class' => 'form-label']) }}
                        <span class="required"></span>
                        {{ Form::text('instruction_date', null, ['class' => 'form-control instructionDate bg-white min-w-170', 'autocomplete' => 'off', 'required', 'id' => 'editConsultantInstructionDate', 'placeholder' => __('messages.ipd_patient_consultant_register.instruction_date')]) }}
                    </div>
                    <div class="form-group col-md-12 mb-5">
                        {{ Form::label('instruction', __('messages.ipd_patient_consultant_register.instruction').':',['class' => 'form-label']) }}
                        <span class="required"></span>
                        {{ Form::textarea('instruction', null, ['class' => 'form-control min-w-170', 'rows' => 4, 'required', 'id' => 'editConsultantInstruction', 'onkeypress' => 'return avoidSpace(event);', 'placeholder' => __('messages.ipd_patient_consultant_register.instruction')]) }}
                    </div>
                </div>
